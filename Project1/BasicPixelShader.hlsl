...
//定数バッファ1
//マテリアル用
cbuffer PMDMaterial : register(b2) {
	float4 pmddiffuse;//ディフューズ色
	float4 pmdspecular;//スペキュラ
	float3 pmdambient;//アンビエント
};
...
float4 BasicPS(BasicType input) : SV_TARGET{
	...
	float specularB = pow(saturate(dot(refLight, -input.ray)),pmdspecular.a);
	...
	return saturate(toonDif//輝度(トゥーン)
		...
		* sph.Sample(smp, sphereMapUV))//スフィアマップ(乗算)
		...
		+ float4(specularB *pmdspecular.rgb, 1))//スペキュラー
		+ float4(texColor*pmdambient*0.5,1);//アンビエント(明るくなりすぎるので0.5にしてます)
